...
const questConditionSchema = object({
  id: number().required().integer().positive(),
  questStepId: number().required().positive(),
  ...
  conditionIndex: number().required().integer().positive(),
  maxValue: number().required().integer().positive(),
...
async function getRemoteFromCache(id: string) {
  const input = `https://esoitem.uesp.net/viewlog.php?record=questCondition&filter=questId&filterid=${id}`;
...
async function parseContent(
  ...
  return selectAll("tr", table)
    ...
    .map((node) => {
      ...
      obj.nameEn = obj.name;
      return questConditionSchema.cast(obj) as InferType<typeof questConditionSchema>;
    });
}

async function saveToStrapi(data: InferType<typeof questConditionSchema>, local: boolean) {
  ...
  const response = await fetch(
    `${local ? "http://localhost:1337/api/" : "https://esoapi.denohub.com/api/"}conditions`,
    {
      method: "POST",
      headers,
  ...
  for (let i = 0; i < 50; i++) {
    ...
    for (const id of questIds) {
      ...
      for (const data of result) {
        try {
          data.quest = data.questId;
          data.step = data.questStepId;
