...
if (import.meta.main) {
  ...
  for (let i = 0; i < 3000; i++) {
    const quest = await fetch(
      "https://esoapi.denohub.com/api/quests?fields[0]=id&fields[0]=nameEn&pagination[pageSize]=500&pagination[page]=1&filters[type][$null]=true&filters[repeatType][$eq]=Not Repeatable",
    ).then((x) => x.json());
